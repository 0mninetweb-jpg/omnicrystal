...
export function CoverageGapCard({
  ...
  const meta = item.card ? getForecastMetaCopy(item.card) : null;
  const groundedLean = item.state === 'grounded_lean';
  return (
    <article className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-[0_18px_44px_rgba(15,23,42,0.05)]">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">{groundedLean ? 'Grounded lean' : 'Coverage gap'}</div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-3">
        <div className="rounded-3xl border border-slate-200 bg-slate-50 p-4">
          <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">{groundedLean ? 'What is still thin' : 'What is missing'}</div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-slate-50 p-4">
          <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">{groundedLean ? 'What could flip it' : 'How to refine'}</div>
          <div className="mt-3 space-y-2 text-sm leading-7 text-slate-700">
        ...
        <div className="rounded-3xl border border-slate-200 bg-slate-50 p-4">
          <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">{groundedLean ? 'How to refine' : 'Alternatives'}</div>
          <div className="mt-3 space-y-2 text-sm leading-7 text-slate-700">
